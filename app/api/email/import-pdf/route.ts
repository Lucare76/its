...
export async function POST(request: NextRequest) {
  try {
    ...
    const subject = String(form.get("subject") ?? `Import PDF manuale ${file.name}`).slice(0, 240);
    const fromEmailRaw = String(form.get("from_email") ?? "operator@manual-upload.local").trim().toLowerCase();
    const fromEmail = /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(fromEmailRaw) ? fromEmailRaw : "operator@manual-upload.local";
    const bodyText = String(form.get("body_text") ?? "Import manuale da PDF in inbox.").trim().slice(0, 200_000);
    ...
    return NextResponse.json({
      ...result,
      id:
        "inbound_email_id" in result && typeof result.inbound_email_id === "string"
          ? result.inbound_email_id
          : null
    });
  } catch (error) {
    auditLog({ event: "pdf_import_route_unhandled", level: "error", details: { message: error instanceof Error ? error.message : "Unknown error" } });
