...
export async function GET(request: NextRequest) {
  try {
    ...
    const { data: pricingRows, error: pricingError } = await auth.admin
    ...
    const { data: servicesData, error: servicesError } = await auth.admin
      .from("services")
    ...
    const [{ data: agencies }, { data: routes }] = await Promise.all([
      agencyIds.length ? auth.admin.from("agencies").select("id,name").in("id", agencyIds) : Promise.resolve({ data: [] }),
      routeIds.length ? auth.admin.from("routes").select("id,name").in("id", routeIds) : Promise.resolve({ data: [] })
    ]);
