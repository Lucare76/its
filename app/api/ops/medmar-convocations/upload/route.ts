...
import { buildMedmarConvocationTemplateParams, buildMedmarConvocationPreviewText } from "@/lib/medmar-convocation-template";
import { auditLog } from "@/lib/server/ops-audit";

export const runtime = "nodejs";

// travelDateLabel/pickupTime/vesselTime arrive already formatted by the
// client (lib/medmar-convocation-format.ts), since only the browser has the
...
const rowSchema = z.object({
  ...
  travelDateLabel: z.string(),
  travelDateIso: z.string().nullable().optional().default(null),
  hotel: z.string(),
  passengers: z.string(),
  pickupTime: z.string(),
  vesselTime: z.string(),
...
export async function POST(request: NextRequest) {
  ...
  for (const row of rows) {
    try {
      phoneKeys.add(normalizeE164(row.phoneRaw));
    } catch { /* invalid numbers handled per-row below */ }
  }
  ...
  if (phoneKeys.size > 0) {
    const { data: sentRows } = await auth.admin
      ...
      .select("phone_e164, travel_date_iso, travel_date, departure_time")
      .eq("tenant_id", tenantId)
      .eq("status", "inviato")
      .in("phone_e164", Array.from(phoneKeys));
    for (const r of sentRows ?? []) {
      const dateKey = r.travel_date_iso ?? String(r.travel_date).trim().toLowerCase();
      previouslySent.add(`${r.phone_e164}||${dateKey}||${String(r.departure_time).trim().toLowerCase()}`);
    }
  }
  ...
  const dbRows = rows.map((row) => {
    ...
    const templateInput = {
      ...
      departureDateLabel: row.travelDateLabel.trim(),
      hotel: row.hotel.trim(),
      passengers: row.passengers.trim(),
      pickupTime: row.pickupTime.trim(),
      vesselTime: row.vesselTime.trim(),
    };
    const templateParams = buildMedmarConvocationTemplateParams(templateInput);
    ...
    const base = {
      ...
      travel_date: templateInput.departureDateLabel,
      travel_date_iso: row.travelDateIso,
      hotel: templateInput.hotel,
      passengers: templateInput.passengers,
      pickup_time: templateInput.pickupTime,
      departure_time: templateInput.vesselTime,
    ...
    const dateKey = row.travelDateIso ?? templateInput.departureDateLabel.toLowerCase();
  ...
  auditLog({
    event: "medmar_convocation_batch_uploaded",
    tenantId,
    userId,
