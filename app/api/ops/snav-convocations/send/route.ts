...
import { DEFAULT_SNAV_CONVOCATION_TEMPLATE, buildSnavConvocationTemplateParams, hasValidSnavTemplateParamCount } from "@/lib/snav-convocation-template";
...
const bodySchema = z.object({
  ...
  templateName: z.string().optional().default(DEFAULT_SNAV_CONVOCATION_TEMPLATE),
...
export async function POST(request: NextRequest) {
  ...
  // Sequential send (bounded concurrency of 1) — matches /medmar-convocations
  ...
  for (const row of rows) {
    ...
    const variables: Record<string, string> = (row.template_payload as Record<string, string> | null) ?? buildSnavConvocationTemplateParams({
      customerName: String(row.customer_name ?? ""),
      departureDateLabel: String(row.departure_date_label ?? ""),
      hotel: String(row.hotel ?? ""),
      passengers: String(row.passengers ?? ""),
      pickupTime: String(row.pickup_time ?? ""),
      vesselTime: String(row.vessel_time ?? ""),
    });

    const nowIso = new Date().toISOString();

    // Never send with a wrong parameter count — the Meta template expects
    // exactly 6 ({{1}}..{{6}}). Mark the row as an error and move on without
    // calling the WhatsApp API.
    if (!hasValidSnavTemplateParamCount(variables)) {
      await auth.admin
        .from("snav_convocation_rows")
    ...
    try {
      await logWhatsAppEvent(auth.admin, {
        ...
        kind: "snav_convocazione",
        template: templateName,
        status: result.ok ? "sent" : "failed",
        provider_message_id: result.ok ? (result.messageId ?? null) : null,
        happened_at: nowIso,
        payload_json: {
          source: "snav-convocations/send",
          snav_convocation_row_id: row.id,
          batch_id: batchId,
  ...
  auditLog({
    event: "snav_convocation_batch_sent",
    tenantId,
    userId,
