...
export async function POST(request: NextRequest) {
  ...
  if (body.action === "delete_bus_reservation") {
    ...
    // caso sarebbe indistinguibile da un successo reale, nonostante la SELECT
    // sopra avesse trovato la riga.
    ...
    if (!deletedRows || deletedRows.length === 0) {
      return NextResponse.json({ ok: false, error: "Cancellazione non riuscita: nessuna riga rimossa." }, { status: 500 });
    }
