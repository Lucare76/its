...
async function ensureAgencyForUser(params: {
  ...
  const { admin, tenantId, userId, fullName, email, currentAgencyId } = params;
  const supportsSetupRequired = await hasColumn(admin, "agencies", "setup_required");
  const externalCode = `auth_user:${userId}`;
  ...
  const existingAgency = currentAgencyId
    ? await admin
        .from("agencies")
        .select("id")
  ...
  if (existingAgency.error) {
    throw new Error(existingAgency.error.message);
  }

  if (existingAgency.data?.id) {
    const agencyUpdate = await admin
      .from("agencies")
      .update(upsertPayload)
      .eq("tenant_id", tenantId)
      .eq("id", existingAgency.data.id)
      .select("id")
      .maybeSingle();
    if (agencyUpdate.error || !agencyUpdate.data?.id) {
      throw new Error(agencyUpdate.error?.message ?? "Aggiornamento agenzia fallito.");
    }
    return agencyUpdate.data.id;
  }

  const agencyInsert = await admin.from("agencies").insert(upsertPayload).select("id").maybeSingle();
  if (agencyInsert.error || !agencyInsert.data?.id) {
    throw new Error(agencyInsert.error?.message ?? "Creazione agenzia fallita.");
  }
  return agencyInsert.data.id;
}
...
export async function POST(request: NextRequest) {
  ...
  // Controlla se esiste una membership con lo stesso nome nel tenant corrente
  ...
  const defaultCapabilities = (Object.entries(capabilityRoleMap) as Array<[AppCapability, string[]]>)
    .filter(([, roles]) => roles.includes(parsed.data.role))
    .map(([capability]) => ({
      tenant_id: auth.membership.tenant_id,
      role: parsed.data.role,
      capability,
  ...
  if (defaultCapabilities.length > 0) {
    const capabilityInsert = await auth.admin.from("role_capability_overrides").insert(defaultCapabilities);
    if (capabilityInsert.error) {
      console.error(
        `Errore nell'inserimento delle capability di default per il nuovo ${parsed.data.role} ${newUserId}:`,
        capabilityInsert.error
      );
      // Non blochiamo la creazione utente se le capability non vengono inserite
    }
  }
...
export async function PATCH(request: NextRequest) {
  ...
  if (parsed.data.role === "agency") {
    const userResult = await auth.admin.auth.admin.getUserById(parsed.data.user_id);
    const targetEmail = userResult.data.user?.email?.trim().toLowerCase() ?? null;
    if (userResult.error || !targetEmail) {
      return NextResponse.json({ error: userResult.error?.message ?? "Email utente non disponibile per il profilo agenzia." }, { status: 400 });
    }

    try {
      nextAgencyId = await ensureAgencyForUser({
        admin: auth.admin,
        tenantId: auth.membership.tenant_id,
        userId: parsed.data.user_id,
        fullName: parsed.data.full_name.trim(),
        email: targetEmail,
        currentAgencyId: membershipLookup.data.agency_id ?? null
      });
    } catch (error) {
      return NextResponse.json(
        { error: error instanceof Error ? error.message : "Preparazione agenzia fallita." },
        { status: 500 }
      );
    }
  }
...
export async function DELETE(request: NextRequest) {
  ...
  if (otherMembershipsLookup.error) {
    return NextResponse.json({ error: otherMembershipsLookup.error.message }, { status: 500 });
  }
