            ...
            {(() => {
              ...
              return source === "pdf" && meta ? (
                ...
                <div className="mt-2 flex flex-wrap gap-2">
                  ...
                  {(() => {
                    const kindMap: Record<string, { label: string; className: string }> = {
                      formula_snav: { label: "SNAV", className: "bg-indigo-100 text-indigo-700" },
                      formula_medmar: { label: "MEDMAR", className: "bg-sky-100 text-sky-700" },
                      transfer_airport_hotel: { label: "Aeroporto", className: "bg-amber-100 text-amber-700" },
                      transfer_train_hotel: { label: "Stazione", className: "bg-orange-100 text-orange-700" },
                      bus_city_hotel: { label: "Bus", className: "bg-emerald-100 text-emerald-700" },
                      excursion: { label: "Escursione", className: "bg-purple-100 text-purple-700" },
                      transfer_port_hotel: { label: "Porto", className: "bg-slate-100 text-slate-700" }
                    };
                    const kind = selectedService.booking_service_kind;
                    const entry = kind ? (kindMap[kind] ?? { label: kind, className: "bg-slate-100 text-slate-700" }) : null;
                    return entry ? (
                      <span className={`rounded-full px-2 py-1 font-semibold uppercase ${entry.className}`}>{entry.label}</span>
                    ) : null;
                  })()}
