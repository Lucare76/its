...
const capabilityLabels = [
  ...
  ,{ capability: "control_room:view", label: "Control Room" }
] as const;
...
export default function SettingsUsersPage() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      if (response.status === 403) {
        setLoading(false);
        setMessage("Accesso negato: servono permessi admin o supervisor.");
        return;
      }
      if (!response.ok) {
        setLoading(false);
  ...
  const reviewAccessRequest = async (request: PendingAccessRequestRow, action: "approve" | "reject") => {
    ...
    if (approvedRequest) {
      setMemberships((prev) => {
        const existingIndex = prev.findIndex((item) => item.user_id === approvedRequest.user_id);
        const nextRow: MembershipRow = {
          ...
          created_at: existingIndex >= 0 ? prev[existingIndex]?.created_at ?? new Date().toISOString() : new Date().toISOString(),
          suspended: false
        };
              ...
              <article key={`tenant-user-${membership.user_id}`} className="overflow-hidden rounded-3xl border border-slate-200 bg-gradient-to-br from-white to-slate-50/90 p-5 shadow-sm">
                ...
                <div className="mt-5 grid gap-3 sm:grid-cols-2 xl:grid-cols-12 xl:items-end">
                  ...
                  <div className="grid gap-1 xl:col-span-1">
                    ...
                    <button
                      ...
                      onClick={() => {
                        ...
                        const gen = genderDrafts[membership.user_id];
                        if (pwd) {
                          void updateMembership(membership, membership.role, pwd, membership.suspended ?? false);
                        }
                        if (gen) {
                          void updateMembership(membership, membership.role, undefined, membership.suspended ?? false, gen);
                        }
                      }}
                    >
                      {updatingUserId === membership.user_id ? "..." : "Salva"}
                    </button>
                  </div>
                  ...
                  <div className="grid gap-1 xl:col-span-1">
                    ...
                    >
                      {sendingResetUserId === membership.user_id ? "..." : "Reset"}
                    </button>
                  </div>
                  ...
                  <div className="grid gap-1 xl:col-span-2">
                    ...
                    >
                      {deletingUserId === membership.user_id ? "..." : "Elimina"}
                    </button>
                  </div>
                </div>
              </article>
