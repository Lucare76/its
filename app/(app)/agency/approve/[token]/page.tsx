  ...
  if (state.kind === "error") {
    return (
      <div className="flex min-h-screen items-center justify-center bg-slate-50 p-4">
        <div className="w-full max-w-md rounded-2xl bg-white p-8 shadow-sm border border-border text-center space-y-3">
  ...
  return (
    <div className="min-h-screen bg-slate-50 py-10 px-4">
      <div className="mx-auto max-w-lg space-y-5">
        ...
        {/* Riepilogo servizio */}
        <div className="rounded-2xl bg-white border border-border p-5 shadow-sm space-y-4">
          ...
          <dl className="grid grid-cols-2 gap-x-4 gap-y-3 text-sm">
            ...
            {service.customer_email && (
              <div className="col-span-2">
                <dt className="text-xs text-slate-400 font-medium uppercase tracking-wide">Email</dt>
        ...
        <div className="rounded-2xl bg-white border border-border p-5 shadow-sm space-y-3">
          ...
          <label className="block text-sm">
            ...
            <input type="email" className="input-saas mt-1" placeholder="operatore@ischiatransferservice.it" value={resolvedByEmail} onChange={e => setResolvedByEmail(e.target.value)} />
          </label>
        </div>
          ...
          <div className="rounded-2xl border-2 border-slate-200 bg-white p-5 space-y-3">
            ...
            <div className="flex gap-3">
              <button type="button" onClick={() => setConfirmStep(null)} disabled={submitting}
                className="flex-1 rounded-xl border border-border px-4 py-2.5 text-sm font-semibold text-slate-600 hover:bg-slate-50 disabled:opacity-50">
                Annulla
              </button>
