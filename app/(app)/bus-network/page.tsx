...
export default function BusNetworkPage() {
  ...
  // --- Derived data ---
  const isAdmin = payload.user_role === "admin" || payload.user_role === "supervisor";
                                  ...
                                  <div key={alloc.id}>
                                    ...
                                    <div
                                      ...
                                      <div className="mt-0.5 flex items-center gap-1">
                                        <span className={`truncate ${alloc.hotel_name === "Hotel N/D" ? "text-rose-400 font-medium" : "text-slate-400"}`}>{alloc.hotel_name}</span>
                                            ...
                                            <div className="space-y-1">
                                              ...
                                              {hotelMatches.map(h => (
                                                <button key={h.id} onClick={() => { void post("set_service_hotel", { allocation_id: alloc.id, hotel_id: h.id }); setEditHotelAllocId(null); }} className="w-full rounded border border-slate-200 bg-white px-2 py-1 text-left text-[10px] hover:bg-violet-50">
                                                  <span className="font-medium">{h.name}</span> <span className="text-slate-400">({h.zone})</span>
                                                </button>
                                              ))}
                                              <div className="flex gap-1 pt-0.5">
