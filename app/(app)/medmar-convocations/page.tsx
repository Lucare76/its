...
type ConvocationRow = {
  id: string;
  /** present only on rows generated from the management system (SPRINT MEDMAR STEP 2). */
  service_id?: string;
  row_index: number;
  inviare: boolean;
  phone_raw: string;
  ...
  sent_at: string | null;
  /** STEP 2 coverage — only present on rows from generate-from-services. */
...
export default function MedmarConvocationsPage() {
  ...
  // "Genera dal gestionale" — read-only preview + coverage (SPRINT MEDMAR STEP 2).
  ...
  const generateFromServices = useCallback(async () => {
    ...
    try {
      ...
      // "Reinvia aggiornate" actually creates one (SPRINT MEDMAR STEP 2).
      setBatchId(null);
      setBatchMeta(null);
      ...
      setExpandedRow(null);
      setStep("preview");
      showToast(`Partenze MEDMAR previste: ${data.summary?.found ?? 0}`);
    } catch (err) {
  ...
  // SPRINT MEDMAR STEP 2 — "Invia solo nuove" / "Reinvia aggiornate": creates
  // a real batch from the gestionale preview (server recomputes coverage,
  // never trusts client state) then reuses the exact same /send pipeline.
  const sendGeneratedBatch = useCallback(async (mode: "new" | "changed") => {
    ...
    try {
      const headers = await authHeaders();
      const createRes = await fetch("/api/ops/medmar-convocations/create-batch-from-services", {
        method: "POST",
        headers,
      ...
      const sendRes = await fetch("/api/ops/medmar-convocations/send", {
        method: "POST",
        headers,
  ...
  return (
    <div className="space-y-6 pb-12">
      <PageHeader
        ...
        subtitle="Invio massivo convocazioni traversate MEDMAR via WhatsApp da file Excel o dal gestionale"
        ...
        <>
          {isGestionalePreview && genSummary ? (
            <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-5">
              <StatCard label="Partenze previste" value={String(genSummary.found)} hint="Servizi MEDMAR del giorno" />
          ...
          >
            ...
            {confirmSendNew && (
              <div className="mb-4 rounded-lg border border-blue-200 bg-blue-50 p-4">
                <p className="text-sm font-medium text-blue-900">
                  Stai per inviare {newCount} nuove convocazioni MEDMAR.
                </p>
                <div className="mt-3 flex gap-2">
            ...
            {confirmSendChanged && (
              <div className="mb-4 rounded-lg border border-amber-200 bg-amber-50 p-4">
                <p className="text-sm font-medium text-amber-900">
                  Stai per reinviare {changedCount} convocazioni MEDMAR i cui dati sono cambiati dopo l&apos;ultimo invio.
                </p>
                <div className="mt-3 flex gap-2">
