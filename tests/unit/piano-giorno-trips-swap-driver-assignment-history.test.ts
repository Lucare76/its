 ...
 * swap_driver di piano-giorno/trips.
 *
 * swap_driver aggiorna assignments.driver_user_id in blocco per tutti i
 * trip_groups attivi del driver sorgente in una data, ma non registrava la
 * variazione tramite logAssignmentChange. Il fix riusa esattamente lo stesso
 * contratto già validato in move_services/assign-service/departure-bus-assign
 * (changeType "driver_swap"), con uno snapshot "prima" tenant-scoped letto
 * subito prima della mutazione bulk. A differenza di move_services/
 * assign-service, swap_driver riceve solo driver_user_id (mai
 * driver_profile_id dal body): il "prima" arriva dallo snapshot assignments,
 * il "dopo" da un lookup best-effort su driver_profiles per to_driver_id —
 * stesso schema già usato in departure-bus-assign.
 *
...
const DRIVER_TO = "d1111111-1111-4111-8111-111111111111";
const DRIVER_TO_NO_PROFILE = "d2222222-2222-4222-8222-222222222222";
const DRIVER_TO_B = "d3333333-3333-4333-8333-333333333333";
const PROFILE_FROM = "p0000000-0000-4000-8000-000000000000";
const PROFILE_TO = "p1111111-1111-4111-8111-111111111111";
 ...
 * piano-giorno-trips-swap-driver-{tenant,status}-guard.test.ts e
...
function baseSeed(overrides: Parameters<typeof createTenantAwareSupabase>[0] = {}) {
  return createTenantAwareSupabase({
    ...
    trip_groups: [
      { id: GROUP_1, tenant_id: TENANT_A, date: TEST_DATE, driver_user_id: DRIVER_FROM, driver_profile_id: PROFILE_FROM, vehicle_label: "Van 8", status: "active" },
      { id: GROUP_INACTIVE, tenant_id: TENANT_A, date: TEST_DATE, driver_user_id: DRIVER_FROM, driver_profile_id: PROFILE_FROM, vehicle_label: "Van 9", status: "cancelled" },
      { id: GROUP_OTHER_DATE, tenant_id: TENANT_A, date: OTHER_DATE, driver_user_id: DRIVER_FROM, driver_profile_id: PROFILE_FROM, vehicle_label: "Van 10", status: "active" },
      { id: GROUP_TENANT_B, tenant_id: TENANT_B, date: TEST_DATE, driver_user_id: DRIVER_FROM, driver_profile_id: null, vehicle_label: "Van B", status: "active" },
    ],
    assignments: [
      { id: "asg-1", tenant_id: TENANT_A, service_id: SERVICE_1, group_id: GROUP_1, driver_user_id: DRIVER_FROM, driver_profile_id: PROFILE_FROM, vehicle_label: "Van 8" },
    ],
    memberships: [
      membershipRow(DRIVER_FROM, TENANT_A),
      membershipRow(DRIVER_TO, TENANT_A),
      membershipRow(DRIVER_TO_NO_PROFILE, TENANT_A),
      membershipRow(DRIVER_TO_B, TENANT_B),
    ],
    driver_profiles: [
      driverProfileRow(PROFILE_FROM, TENANT_A, DRIVER_FROM),
      driverProfileRow(PROFILE_TO, TENANT_A, DRIVER_TO),
    ],
    driver_daily_availability: [
      { tenant_id: TENANT_A, driver_user_id: DRIVER_FROM, date: TEST_DATE, available: true, available_from: "00:00", available_to: "23:59" },
      { tenant_id: TENANT_A, driver_user_id: DRIVER_TO, date: TEST_DATE, available: true, available_from: "00:00", available_to: "23:59" },
      { tenant_id: TENANT_A, driver_user_id: DRIVER_TO_NO_PROFILE, date: TEST_DATE, available: true, available_from: "00:00", available_to: "23:59" },
      { tenant_id: TENANT_A, driver_profile_id: PROFILE_FROM, date: TEST_DATE, available: true, available_from: "00:00", available_to: "23:59" },
      { tenant_id: TENANT_A, driver_profile_id: PROFILE_TO, date: TEST_DATE, available: true, available_from: "00:00", available_to: "23:59" },
    ],
...
describe("CONC-07 — storico strutturato (driver_assignment_history) su swap_driver", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  it("1. swap singolo gruppo: history scritto, changeType driver_swap", async () => {
    const fake = baseSeed();
    authorizeAs(fake);
  ...
  it("2. swap più gruppi: history per ogni servizio", async () => {
    const fake = baseSeed({
      trip_groups: [
        { id: GROUP_1, tenant_id: TENANT_A, date: TEST_DATE, driver_user_id: DRIVER_FROM, driver_profile_id: PROFILE_FROM, vehicle_label: "Van 8", status: "active" },
        { id: GROUP_2, tenant_id: TENANT_A, date: TEST_DATE, driver_user_id: DRIVER_FROM, driver_profile_id: PROFILE_FROM, vehicle_label: "Van 9", status: "active" },
      ],
      assignments: [
        { id: "asg-1", tenant_id: TENANT_A, service_id: SERVICE_1, group_id: GROUP_1, driver_user_id: DRIVER_FROM, driver_profile_id: PROFILE_FROM, vehicle_label: "Van 8" },
        { id: "asg-2", tenant_id: TENANT_A, service_id: SERVICE_2, group_id: GROUP_2, driver_user_id: DRIVER_FROM, driver_profile_id: PROFILE_FROM, vehicle_label: "Van 9" },
      ],
    });
    authorizeAs(fake);
  ...
  it("3. più servizi nello stesso gruppo: entry per ciascuno, stesso groupId", async () => {
    const fake = baseSeed({
      services: [serviceRow(SERVICE_1), serviceRow(SERVICE_2, { time: "14:00:00" }), serviceRow(SERVICE_3)],
      assignments: [
        { id: "asg-1", tenant_id: TENANT_A, service_id: SERVICE_1, group_id: GROUP_1, driver_user_id: DRIVER_FROM, driver_profile_id: PROFILE_FROM, vehicle_label: "Van 8" },
        { id: "asg-2", tenant_id: TENANT_A, service_id: SERVICE_2, group_id: GROUP_1, driver_user_id: DRIVER_FROM, driver_profile_id: PROFILE_FROM, vehicle_label: "Van 8" },
      ],
    });
    authorizeAs(fake);

    await callPost(swapDriverBody());
  ...
  it("4. previous driver/profile corretti", async () => {
    const fake = baseSeed();
    authorizeAs(fake);
    ...
    const entries = lastHistoryEntries();
    expect(entries[0].fromDriverProfileId).toBe(PROFILE_FROM);
  });

  it("5. target driver/profile corretti", async () => {
    const fake = baseSeed();
    authorizeAs(fake);
    ...
    const entries = lastHistoryEntries();
    expect(entries[0].toDriverProfileId).toBe(PROFILE_TO);
  });

  it("6. vehicle invariato: from === to (swap_driver non tocca il mezzo)", async () => {
    const fake = baseSeed();
    authorizeAs(fake);
    ...
    expect(entries[0].toVehicleLabel).toBe("Van 8");
  });

  it("7. groupId corretto nell'entry", async () => {
    const fake = baseSeed();
    authorizeAs(fake);
  ...
  it("8. actor corretto: operatorId coincide con l'utente autenticato", async () => {
    const fake = baseSeed();
  ...
  it("9. tenant corretto: tenantId coincide con quello della sessione", async () => {
    const fake = baseSeed();
    authorizeAs(fake);
  ...
  it("10. tenant_id malevolo nel body viene ignorato: history usa il tenant di sessione", async () => {
    const fake = baseSeed();
    authorizeAs(fake);

    await callPost(swapDriverBody({ tenant_id: TENANT_B }));
  ...
  it("11. gruppo di tenant B escluso: nessuna entry lo riguarda", async () => {
    const fake = baseSeed();
    authorizeAs(fake);
  ...
  it("12. gruppo di altra data escluso: nessuna entry lo riguarda", async () => {
    const fake = baseSeed();
    authorizeAs(fake);
    ...
    expect(groupOtherDate?.driver_user_id).toBe(DRIVER_FROM);
  });

  it("13. gruppo non attivo (status cancelled) escluso: nessuna entry lo riguarda", async () => {
    const fake = baseSeed();
    authorizeAs(fake);
  ...
  it("15. nessun gruppo trovato (from_driver_id senza giri attivi): comportamento invariato, zero history", async () => {
    const fake = baseSeed({
      trip_groups: [],
      assignments: [],
    });
  ...
  it("16. stesso driver sorgente/destinazione: nessun evento inutile (comunque nessuna history, indipendentemente dall'esito della validazione preesistente)", async () => {
    // to_driver_id === from_driver_id fa rilevare a validateTripPayload
    // (preesistente, non toccato da questo fix) un conflitto con
    // l'assignment/gruppo già esistente dello stesso driver — comportamento
    // di validazione fuori scope qui. L'unica cosa rilevante per CONC-07 è
    // che, in ogni caso, non viene mai prodotto un evento driver_swap inutile
    // quando from === to.
    const fake = baseSeed();
    authorizeAs(fake);

    await callPost(swapDriverBody({ to_driver_id: DRIVER_FROM }));

    expect(mocks.logAssignmentChange).not.toHaveBeenCalled();
  });

  it("17. profilo sorgente assente: comportamento conservativo, fromDriverProfileId null (nessun valore inventato)", async () => {
    const fake = baseSeed({
      assignments: [
        { id: "asg-1", tenant_id: TENANT_A, service_id: SERVICE_1, group_id: GROUP_1, driver_user_id: DRIVER_FROM, driver_profile_id: null, vehicle_label: "Van 8" },
      ],
    });
    authorizeAs(fake);
  ...
  it("18. profilo target assente: comportamento conservativo, toDriverProfileId null (nessun valore inventato)", async () => {
    const fake = baseSeed();
    authorizeAs(fake);

    await callPost(swapDriverBody({ to_driver_id: DRIVER_TO_NO_PROFILE }));

    const entries = lastHistoryEntries();
    expect(entries[0].toDriverProfileId).toBeNull();
    expect(entries[0].fromDriverProfileId).toBe(PROFILE_FROM);
  });

  it("19. nessun history su 401 (sessione non valida)", async () => {
  ...
  it("20. nessun history su 403 (ruolo non autorizzato)", async () => {
  ...
  it("21. nessun history su DRIVER_NOT_FOUND (target cross-tenant, SEC-05)", async () => {
    const fake = baseSeed();
    authorizeAs(fake);
  ...
  it("22. nessun history su DRIVER_NOT_ACTIVE (target sospeso, FUNC-03)", async () => {
    const fake = baseSeed({
      memberships: [
        membershipRow(DRIVER_FROM, TENANT_A),
        membershipRow(DRIVER_TO, TENANT_A, { suspended: true }),
      ],
    });
    authorizeAs(fake);
  ...
  it("23. nessun history su fallimento availability/timeline (validateTripPayload rifiuta)", async () => {
    const fake = baseSeed({
      driver_daily_availability: [
        { tenant_id: TENANT_A, driver_user_id: DRIVER_FROM, date: TEST_DATE, available: true, available_from: "00:00", available_to: "23:59" },
        { tenant_id: TENANT_A, driver_user_id: DRIVER_TO, date: TEST_DATE, available: false, available_from: "00:00", available_to: "23:59" },
      ],
    });
    authorizeAs(fake);
  ...
  it("25. nessun history se l'update di assignments fallisce", async () => {
    const fake = baseSeed();
  ...
  it("26. nessun history se l'update di trip_groups fallisce", async () => {
    const fake = baseSeed();
  ...
  it("27. snapshot previous fallisce: swap riuscito, zero history", async () => {
    const fake = baseSeed();
    // 1a select assignments: loadGroupServiceIds; 2a: validateTripPayload
    // (otherAssignments/timeline); 3a: lo snapshot CONC-07 sotto test.
    fake.setError("assignments", RAW_DB_ERROR, 3);
    authorizeAs(fake);
  ...
  it("28. query driver_profiles fallisce: swap riuscito, zero history", async () => {
    const fake = baseSeed();
    // 1a select driver_profiles: risoluzione profileId dentro
    // validateDriverAvailabilityPayload (preesistente, deve continuare a
    // funzionare); 2a: il lookup CONC-07 sotto test.
  ...
  it("31. push invariato su successo: sendPushToUser chiamato una volta verso to_driver_id", async () => {
    const fake = baseSeed();
    authorizeAs(fake);
    ...
    expect(mocks.sendPushToUser).toHaveBeenCalledWith(TENANT_A, DRIVER_TO, expect.any(Object));
  });

  it("32. zero push sui rifiuti (target cross-tenant)", async () => {
    const fake = baseSeed();
    authorizeAs(fake);
  ...
  it("33. risposta HTTP invariata: stessa forma { ok, affected, warnings }", async () => {
    const fake = baseSeed();
    authorizeAs(fake);
  ...
  it("34. create_trip invariata: la modifica a swap_driver non lo tocca", async () => {
    const fake = baseSeed({
  ...
  it("35. update_trip invariata: la modifica a swap_driver non lo tocca", async () => {
    const fake = baseSeed();
    authorizeAs(fake);

    const res = await callPost({ action: "update_trip", group_id: GROUP_1, driver_user_id: DRIVER_TO, service_ids: [SERVICE_1] });
    const body = await res.json();
  ...
  it("36. move_services invariata: la modifica a swap_driver non lo tocca", async () => {
    const fake = baseSeed({
      trip_groups: [
        { id: GROUP_1, tenant_id: TENANT_A, date: TEST_DATE, driver_user_id: DRIVER_FROM, driver_profile_id: PROFILE_FROM, vehicle_label: "Van 8", status: "active" },
        { id: GROUP_2, tenant_id: TENANT_A, date: TEST_DATE, driver_user_id: DRIVER_TO, driver_profile_id: PROFILE_TO, vehicle_label: "Van 9", status: "active" },
      ],
    });
    authorizeAs(fake);
  ...
  it("37. swap_vehicle invariata: la modifica a swap_driver non lo tocca", async () => {
    const fake = baseSeed();
    authorizeAs(fake);
  ...
  it("38. delete_trip invariata: la modifica a swap_driver non lo tocca", async () => {
    const fake = baseSeed();
    authorizeAs(fake);
