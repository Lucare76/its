...
describe("POST /api/ops/new-booking — wiring pickup automatico treno/aeroporto", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    mocks.resolveBookingPracticeNumber.mockResolvedValue("ITS-2026-1");
  });

  it("transfer_train_hotel (return_only, come SUORATO) usa departure_time=14:00 come orario treno -> pickup_hotel 11:00", async () => {
    ...
    await POST(makeRequest(basePayload({ departure_time: "14:00", customer_first_name: "Cristina", customer_last_name: "Suorato" })));

    expect(serviceInserts.length).toBe(1);
    expect(serviceInserts[0]!.direction).toBe("departure");
    expect(serviceInserts[0]!.pickup_hotel).toBe("11:00");
  });

  it("transfer_train_hotel (return_only, come BIRAGO) usa departure_time=12:10 come orario treno -> pickup_hotel 09:00", async () => {
    const serviceInserts: Array<Record<string, unknown>> = [];
    mocks.authorizePricingRequest.mockResolvedValue(makeAuthContext(makeFakeAdmin({ serviceInserts })));

    await POST(makeRequest(basePayload({ departure_time: "12:10", customer_first_name: "Annamaria", customer_last_name: "Birago" })));

    expect(serviceInserts[0]!.pickup_hotel).toBe("09:00");
  });

  it("nessun fallback da departure_time come pickup finale: pickup_hotel e' il risultato del calcolo, non una copia di departure_time", async () => {
    const serviceInserts: Array<Record<string, unknown>> = [];
    mocks.authorizePricingRequest.mockResolvedValue(makeAuthContext(makeFakeAdmin({ serviceInserts })));

    await POST(makeRequest(basePayload({ departure_time: "14:00" })));

    expect(serviceInserts[0]!.pickup_hotel).not.toBe("14:00");
    expect(serviceInserts[0]!.departure_time).toBe("14:00");
  });

  it("arrival (outbound_only) non triggera mai il calcolo pickup anche con kind treno/aeroporto", async () => {
    const serviceInserts: Array<Record<string, unknown>> = [];
    mocks.authorizePricingRequest.mockResolvedValue(makeAuthContext(makeFakeAdmin({ serviceInserts })));

    await POST(makeRequest(basePayload({ trip_leg: "outbound_only" })));

    expect(serviceInserts.length).toBe(1);
    expect(serviceInserts[0]!.direction).toBe("arrival");
    expect(serviceInserts[0]!.pickup_hotel ?? null).toBeNull();
  });

  it("formula_medmar_napoli (ferry, non treno/aereo) non genera pickup_hotel automatico", async () => {
    const serviceInserts: Array<Record<string, unknown>> = [];
    mocks.authorizePricingRequest.mockResolvedValue(makeAuthContext(makeFakeAdmin({ serviceInserts })));
