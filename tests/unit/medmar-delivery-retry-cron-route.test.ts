...
describe("GET /api/cron/medmar-delivery-retry", () => {
  ...
  it("risposta include timedOut e message quando il batch va in timeout", async () => {
    mocks.runMedmarDeliveryRetryBatch.mockResolvedValue({ ...SUMMARY, timed_out: true, processed: 0, delivered: 0 });
    const res = await GET(
      makeRequest("http://localhost:3010/api/cron/medmar-delivery-retry", { authorization: "Bearer test-cron-secret" })
    );
    expect(res.status).toBe(200);
    ...
    expect(body.message).toBe("Retry deferred because processing budget expired");
  });
