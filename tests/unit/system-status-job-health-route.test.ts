...
describe("GET /api/admin/system-status — job health", () => {
  ...
  it("restituisce job_health per il tenant autorizzato senza esporre secret", async () => {
    ...
    vi.mocked(readRecentJobRuns).mockResolvedValue({ backup: [], "poll-emails": [], "postgres-backup": [], "storage-backup": [], "whatsapp-reminders": [] });
    ...
    expect(readSystemJobHealthSummary).toHaveBeenCalledWith(admin, "tenant-1", ["backup", "poll-emails", "postgres-backup", "storage-backup", "whatsapp-reminders"], expect.any(String));
    expect(readRecentJobRuns).toHaveBeenCalledWith(admin, "tenant-1", ["backup", "poll-emails", "postgres-backup", "storage-backup", "whatsapp-reminders"]);
