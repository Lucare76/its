...
const GROUP_TARGET_WITH_SUSPENDED_DRIVER = "c3333333-3333-4333-8333-333333333333";
 ...
 * "memberships"/"driver_profiles": SEC-05 e FUNC-03 interrogano entrambe
 * le stesse tabelle in sequenza (SEC-05 prima, FUNC-03 subito dopo), sia nel
 * ramo nuovo giro sia nel ramo valori finali comune.
 */
...
function baseSeed(overrides: Parameters<typeof createTenantAwareSupabase>[0] = {}) {
  return createTenantAwareSupabase({
    ...
    trip_groups: [
      ...
      { id: GROUP_TARGET_WITH_SUSPENDED_DRIVER, tenant_id: TENANT_A, date: TEST_DATE, driver_user_id: DRIVER_SUSPENDED, driver_profile_id: null, vehicle_label: null, status: "active" },
    ],
    assignments: [
    ...
    driver_profiles: [
      ...
      driverProfileRow(PROFILE_INACTIVE, TENANT_A, DRIVER_VALID, { active: false }),
    ],
...
function moveServicesBody(overrides: Record<string, unknown> = {}) {
  return {
    ...
    target_group_id: GROUP_TARGET_NO_DRIVER,
...
describe("FUNC-03 residuo — guard operatività driver in piano-giorno/trips (move_services, riuso helper)", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
  ...
  it("1. nuovo giro, driver_user operativo: successo", async () => {
    const fake = baseSeed();
    authorizeAs(fake);

    const res = await callPost(moveServicesBody({ target_group_id: undefined, date: TEST_DATE }));
  ...
  it("2. nuovo giro, driver_user sospeso: 409 DRIVER_NOT_ACTIVE, zero trip_group creato", async () => {
    const fake = baseSeed();
    authorizeAs(fake);

    const res = await callPost(moveServicesBody({ target_group_id: undefined, date: TEST_DATE, driver_user_id: DRIVER_SUSPENDED }));
    const body = await res.json();
  ...
  it("3. nuovo giro, driver_profile attivo (coppia valida): successo", async () => {
    const fake = baseSeed();
    authorizeAs(fake);

    const res = await callPost(moveServicesBody({ target_group_id: undefined, date: TEST_DATE, driver_user_id: DRIVER_VALID, driver_profile_id: PROFILE_VALID }));
    const body = await res.json();
  ...
  it("4. nuovo giro, driver_profile inattivo: 409 DRIVER_NOT_ACTIVE, zero trip_group creato", async () => {
    const fake = baseSeed();
    authorizeAs(fake);

    const res = await callPost(moveServicesBody({ target_group_id: undefined, date: TEST_DATE, driver_user_id: DRIVER_VALID, driver_profile_id: PROFILE_INACTIVE }));
    const body = await res.json();
  ...
  it("5. nuovo giro, coppia user+profile entrambi operativi: successo", async () => {
    const fake = baseSeed();
    authorizeAs(fake);
  ...
  it("6. nuovo giro, user operativo + profile inattivo: 409, zero trip_group creato", async () => {
    const fake = baseSeed();
    authorizeAs(fake);
  ...
  it("7. nuovo giro, entrambi assenti: guard FUNC-03 skippato (no-op), comportamento preesistente invariato", async () => {
    const fake = baseSeed();
    authorizeAs(fake);

    const res = await callPost(moveServicesBody({ target_group_id: undefined, date: TEST_DATE, driver_user_id: undefined }));
    const body = await res.json();
  ...
  it("8. gruppo esistente, driver finale operativo: successo", async () => {
    const fake = baseSeed();
    authorizeAs(fake);

    const res = await callPost(moveServicesBody({ target_group_id: GROUP_TARGET_WITH_DRIVER, driver_user_id: undefined }));
    const body = await res.json();
  ...
  it("9. gruppo esistente CON driver già sospeso nel gruppo, nessun driver nel body: guard rivalida il driver del gruppo, 409", async () => {
    const fake = baseSeed();
    authorizeAs(fake);

    const res = await callPost(moveServicesBody({ target_group_id: GROUP_TARGET_WITH_SUSPENDED_DRIVER, driver_user_id: undefined }));
    const body = await res.json();
  ...
  it("10. gruppo esistente senza driver + driver_profile_id inattivo nel body: 409, zero scritture", async () => {
    const fake = baseSeed();
    authorizeAs(fake);

    const res = await callPost(moveServicesBody({ target_group_id: GROUP_TARGET_NO_DRIVER, driver_user_id: DRIVER_VALID, driver_profile_id: PROFILE_INACTIVE }));
    const body = await res.json();
  ...
  it("11. gruppo esistente senza driver (target group senza driver, body senza driver): comportamento corrente invariato (nessun DRIVER_NOT_ACTIVE spurio)", async () => {
    const fake = baseSeed();
    authorizeAs(fake);
  ...
  it("12. gruppo esistente, coppia finale coerente e operativa: successo", async () => {
    const fake = baseSeed();
    authorizeAs(fake);
  ...
  it("13. gruppo esistente, coppia finale non operativa (profile inattivo): 409, zero scritture", async () => {
    const fake = baseSeed();
    authorizeAs(fake);
  ...
  it("14. driver cross-tenant: SEC-05 blocca prima (404 DRIVER_NOT_FOUND), FUNC-03 mai raggiunto", async () => {
    const fake = baseSeed();
    authorizeAs(fake);
  ...
  it("15. driver inesistente: 404 DRIVER_NOT_FOUND (SEC-05), non 409", async () => {
    const fake = baseSeed();
    authorizeAs(fake);
  ...
  it("16. ruolo non-driver: 404 DRIVER_NOT_FOUND (SEC-05), non 409", async () => {
    const fake = baseSeed();
    authorizeAs(fake);
  ...
  it("17. servizio non operativo: FUNC-02 blocca prima di FUNC-03, anche con driver sospeso", async () => {
    const fake = baseSeed({ services: [serviceRow(TENANT_A, SERVICE_A1, { status: "cancelled" })] });
    authorizeAs(fake);

    const res = await callPost(moveServicesBody({ target_group_id: GROUP_TARGET_NO_DRIVER, driver_user_id: DRIVER_SUSPENDED }));
    const body = await res.json();
  ...
  it("18-19. zero scritture/notifiche su rifiuto FUNC-03", async () => {
    const fake = baseSeed();
    authorizeAs(fake);

    await callPost(moveServicesBody({ target_group_id: GROUP_TARGET_NO_DRIVER, driver_user_id: DRIVER_SUSPENDED }));
  ...
  it("20. timeline invariata su successo: warnings presenti, group_id nel body di risposta", async () => {
    const fake = baseSeed();
    authorizeAs(fake);
  ...
  it("21. create_trip invariato: guard su move_services non tocca create_trip", async () => {
    const fake = baseSeed({
  ...
  it("22. update_trip invariato: guard su move_services non tocca update_trip", async () => {
    const fake = baseSeed();
    authorizeAs(fake);
  ...
  it("23. swap_driver invariato: guard su move_services non tocca swap_driver", async () => {
    const fake = baseSeed();
    authorizeAs(fake);
  ...
  it("24. 401: utente non autenticato, guard mai raggiunto", async () => {
  ...
  it("25. 403: ruolo non autorizzato, guard mai raggiunto", async () => {
  ...
  it("26. tenant_id malevolo nel body viene ignorato: verifica operatività contro il tenant di sessione", async () => {
    const fake = baseSeed();
    authorizeAs(fake);
  ...
  it("27. privacy/sanitizzazione: risposta 409 non contiene dettagli DB, driver_id o SQLSTATE", async () => {
    const fake = baseSeed();
    authorizeAs(fake);
    ...
    expect(raw).not.toMatch(new RegExp(DRIVER_SUSPENDED));
    expect(raw.toLowerCase()).not.toMatch(/sqlstate|stack|supabase|postgres/);
  });

  // ─── ERRORE DB / FAIL-CLOSED ──────────────────────────────────────────────

  it("28. errore query memberships (ramo nuovo giro, 2a select 'memberships' dopo SEC-05): 500 fail-closed, zero scritture", async () => {
    const fake = baseSeed();
    fake.setError("memberships", RAW_DB_ERROR, 2);
    authorizeAs(fake);

    const res = await callPost(moveServicesBody({ target_group_id: undefined, date: TEST_DATE, driver_user_id: DRIVER_VALID }));
    const body = await res.json();
  ...
  it("29. errore query driver_profiles (ramo valori finali, dopo SEC-05): 500 fail-closed, zero scritture", async () => {
    const fake = baseSeed();
...
describe("FUNC-03 residuo — sensibilità (comportamento atteso col guard integro, move_services)", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it("30. sensibilità: driver sospeso deve essere rifiutato quando il filtro suspended è integro (ramo nuovo giro)", async () => {
    const fake = baseSeed();
    authorizeAs(fake);

    const res = await callPost(moveServicesBody({ target_group_id: undefined, date: TEST_DATE, driver_user_id: DRIVER_SUSPENDED }));
    expect(res.status).toBe(409);
    expect((await res.json()).error).toBe("DRIVER_NOT_ACTIVE");
  });

  it("31. sensibilità: profilo inattivo deve essere rifiutato quando il filtro active è integro (ramo gruppo esistente)", async () => {
    const fake = baseSeed();
    authorizeAs(fake);
  ...
  it("32. sensibilità: bypass del guard nel ramo nuovo giro deve essere rilevabile (verificato con guard integro: 409 reale)", async () => {
    const fake = baseSeed();
    authorizeAs(fake);
  ...
  it("33. sensibilità: bypass del guard nel ramo gruppo esistente deve essere rilevabile (verificato con guard integro: 409 reale)", async () => {
    const fake = baseSeed();
    authorizeAs(fake);
