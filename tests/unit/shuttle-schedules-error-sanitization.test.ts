...
// un errore durante la transazione (guardia, delete o insert: dal punto di
// vista del chiamante e' indistinguibile, ed e' proprio questo il punto —
// tutto o niente) e' simulato con un unico setRpcError. GET/POST restano
// invariati (select/insert dirette su "services"/"hotels").
  ...
  function makeSelectBuilder(kind: "hotels" | "services", rows: Row[]) {
    ...
    const builder = {
      ...
      then(
        ...
        const error = kind === "hotels" ? hotelsSelectError : servicesSelectError;
...
describe("shuttle-schedules API — error sanitization (F-11 mitigation)", () => {
  ...
  it("3. PATCH: errore nel flusso di aggiornamento → 500, nessun dettaglio tecnico, log invocato, nessuna operazione dopo l'errore", async () => {
    ...
    fake.setRpcError(DISTINCTIVE_ERROR);
    authorizeAs(TENANT_A, fake);
  ...
  it("5. Risposte 500 non contengono mai i campi details/hint/code/stack (verifica esplicita su tutti e 4 i percorsi)", async () => {
    ...
    fakePatch.setRpcError(DISTINCTIVE_ERROR);
    ...
    fakeDelete.setRpcError(DISTINCTIVE_ERROR);
