...
describe("PATCH /api/shuttle-schedules/[id] — valid_from obbligatorio (M1.1.2)", () => {
  ...
  it("valid_from assente → 400, nessuna scrittura Supabase", async () => {
    const { valid_from: _omit, ...payload } = VALID_PAYLOAD;
  ...
  it("valid_from: null → 400, nessuna scrittura Supabase", async () => {
    const res = await callPatch(VALID_SCHEDULE_ID, { ...VALID_PAYLOAD, valid_from: null });
  ...
  it('valid_from: "" → 400, nessuna scrittura Supabase', async () => {
    const res = await callPatch(VALID_SCHEDULE_ID, { ...VALID_PAYLOAD, valid_from: "" });
  ...
  it("valid_from non nel formato YYYY-MM-DD → 400, nessuna scrittura Supabase", async () => {
    const res = await callPatch(VALID_SCHEDULE_ID, { ...VALID_PAYLOAD, valid_from: "01-08-2026" });
  ...
  it("valid_from valida e valid_to valida → supera la validazione, comportamento preesistente invariato (delete + insert eseguiti, 200 ok)", async () => {
  ...
  it("valid_from successiva a valid_to → 400, nessuna scrittura Supabase (controllo preesistente invariato)", async () => {
