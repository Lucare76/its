-- FIX MIRATO GIACOMONI — Obiettivo C: l'agenzia di un gruppo bus_exclusive
-- vive su booking_groups.agency_id, MAI su services.billing_party_name (i
-- service generati da un booking_group non lo valorizzano mai — vedi
-- addBookingGroupPassengers in lib/server/booking-groups-service.ts). La view
-- leggeva solo services.billing_party_name, quindi il PDF/Excel non mostrava
-- mai l'agenzia per un gruppo, anche quando booking_groups.agency_id era
-- valorizzato (caso reale: GRUPPO GIACOMONI, agency_id impostato).
--
-- Fallback (Obiettivo C, regola esplicita del prompt):
--   1. booking_groups.agency_id -> agencies.name (se il service appartiene a
--      un gruppo con agenzia impostata);
--   2. services.billing_party_name (comportamento individuale invariato);
--   3. nessuno dei due -> null, MAI un'agenzia inventata (il fallback
--      "Agenzia N/D"/regex sulle notes resta lato client, invariato).
...
select
  ...
  coalesce(
    nullif(trim(bg_agency.name), ''),
    nullif(trim(s.billing_party_name), '')
  ) as agency_name,
  a.notes,
  a.created_at,
  a.tenant_id,
...
left join public.booking_groups bg on bg.id = s.booking_group_id
left join public.agencies bg_agency on bg_agency.id = bg.agency_id
