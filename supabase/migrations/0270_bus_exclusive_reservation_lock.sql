-- FIX FINALE bus_exclusive A/R — Obiettivo B: un bus riservato in esclusiva
-- per un booking_group (booking_group_bus_reservations.exclusive = true, per
-- tenant_id + bus_unit_id + service_date) deve rifiutare QUALSIASI service di
-- un booking_group diverso (o senza booking_group_id) per quella stessa data.
--
-- Applicato dentro allocate_bus_service e move_bus_allocation: sono l'UNICO
-- punto di scrittura di tenant_bus_allocations (RPC security definer), quindi
-- questo blocco copre TUTTI i percorsi (allocazione manuale, drag/drop,
-- autoAllocateBusService, allocateReservedBookingGroupBusService, smista per
-- zona) senza duplicare la regola in ciascun chiamante TypeScript.
--
-- Un servizio SENZA booking_group_id (individuale) o di un booking_group
-- DIVERSO viene sempre rifiutato su un bus esclusivo riservato da un altro
-- gruppo. Un servizio dello STESSO booking_group_id riservatario resta
-- consentito.
