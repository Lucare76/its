-- FIX MIRATO — CAPIENZA BUS CROSS-DIRECTION (audit produzione 2026-09-04).
--
-- ROOT CAUSE CERTIFICATA: allocate_bus_service e move_bus_allocation
-- calcolano l'occupazione di un tenant_bus_unit sommando pax_assigned per
-- (bus_unit_id, service.date) SENZA filtrare per service.direction. Un bus
-- fisico serve sia il giro arrival sia il giro departure dello stesso
-- giorno: sono due viaggi operativi distinti con capienza propria (54 posti
-- ciascuno), ma il check attuale li tratta come un unico pool condiviso.
--
-- Caso reale 2026-09-06, Linea Centro:
--   CENTRO 1: 50 pax arrival preesistenti -> departure poteva accettare
--             solo 4 pax prima di "Capienza bus superata" (50+X>54).
--   CENTRO 2: 54 pax arrival preesistenti (pieno) -> QUALSIASI departure
--             veniva rifiutata, pur avendo 0 pax departure.
-- Il planner JS (import_excel_auto / planDepartureBusAssignments) calcola
-- correttamente la capienza residua SOLO sulla direction corrente e quindi
-- pianificava in modo corretto (Terni 18 pax su un solo bus) — ma l'RPC,
-- controllando arrival+departure insieme, rifiutava gran parte dei piani,
-- forzando il fallback riga-per-riga a spargere le prenotazioni su altri bus.
--
-- move_bus_allocation aveva GIA' questo fix in 0071_fix_move_allocation_direction.sql
-- ("and s.direction = v_service.direction"), ma 0270_bus_exclusive_reservation_lock.sql
-- ha ridefinito la funzione per aggiungere il lock esclusivo partendo da una
-- versione precedente a 0071, perdendo silenziosamente il filtro direction
-- (regressione). allocate_bus_service non lo ha mai avuto.
--
-- Questa migration ridefinisce ENTRAMBE le funzioni a partire dalla versione
-- attualmente live (0270), aggiungendo solo "and s.direction = v_service.direction"
-- al check di capienza. Tutto il resto — lock esclusivo booking_group,
-- validazione tenant/linea/fermata/bus chiuso, audit su
-- tenant_bus_allocation_moves, messaggi di errore — resta identico, verbatim.
--
-- Nuovo pool di capienza: bus_unit_id + service.date + service.direction.
-- Arrival e departure dello stesso giorno sullo stesso bus non si sommano
-- piu' fra loro.
...
begin
  ...
  -- Obiettivo B: bus riservato in esclusiva da un ALTRO gruppo per questa
  -- data -> rifiuta sempre, anche se il servizio non appartiene a nessun
  -- gruppo (individuale) o appartiene a un gruppo diverso.
  select booking_group_id into v_exclusive_group_id
  from public.booking_group_bus_reservations
  where tenant_id = p_tenant_id
    and bus_unit_id = p_bus_unit_id
    and service_date = v_service.date
    and exclusive = true
  limit 1;
  if v_exclusive_group_id is not null
     and (v_service.booking_group_id is null or v_service.booking_group_id <> v_exclusive_group_id) then
    raise exception 'Bus riservato in esclusiva per un altro gruppo in questa data: nessun altro passeggero puo essere assegnato.';
  end if;
  ...
  -- Controlla capienza per la stessa data E STESSA DIREZIONE del servizio —
  -- arrival e departure dello stesso giorno sono due viaggi operativi
  -- distinti dello stesso bus fisico, con pool di capienza separati (FIX
  -- MIRATO cross-direction, vedi commento in testa alla migration).
  ...
  where a.tenant_id = p_tenant_id
    ...
    and s.direction = v_service.direction;
...
begin
  ...
  -- Obiettivo B: stesso blocco esclusivita' di allocate_bus_service, ma sul
  -- bus di DESTINAZIONE dello spostamento (drag/drop non deve poter bypassare
  -- il lock).
  select booking_group_id into v_exclusive_group_id
  from public.booking_group_bus_reservations
  where tenant_id = p_tenant_id
  ...
  if v_exclusive_group_id is not null
     and (v_service.booking_group_id is null or v_service.booking_group_id <> v_exclusive_group_id) then
    raise exception 'Bus destinazione riservato in esclusiva per un altro gruppo in questa data: spostamento non consentito.';
  end if;

  -- Controlla capienza per la stessa data E STESSA DIREZIONE del servizio
  -- dell'allocazione che si sta spostando (FIX MIRATO cross-direction —
  -- ripristina il filtro gia' introdotto in
  -- 0071_fix_move_allocation_direction.sql e perso nella riscrittura
  -- 0270_bus_exclusive_reservation_lock.sql).
  select coalesce(sum(a.pax_assigned), 0)
  into v_target_bus_pax
  from public.tenant_bus_allocations a
  join public.services s on s.id = a.service_id
