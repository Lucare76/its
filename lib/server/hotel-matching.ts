...
export function normalizeHotelText(value: string | null | undefined) {
  return String(value ?? "")
    ...
    .replace(/['".,]/g, " ")
    .replace(/\b(?:hotel|htl|albergo|terme|thermal|resort|spa|club|grand|park|parco|relax|exclusive|boutique|di|del|della|delle|dei|degli)\b/g, " ")
    .replace(/[^a-z0-9]+/g, " ")
    .replace(/\s+/g, " ")
    .trim();
}
...
function scoreCandidate(wantedRaw: string, candidateRaw: string) {
  ...
  if (wanted.length >= 5 && candidate.length >= 5 && (candidate.includes(wanted) || wanted.includes(candidate))) return 92;
