...
function parseTsfBlocks(compact: string, practiceNumber: string | null) {
  const blocks = Array.from(
    ...
    //         5=dalTable 6=alTable 7=directionRow 8=lineAmount
    //         9=paxOptionA (N\(n\)) 10=paxOptionB (\(N\)) 11=totalAmount
    const passengerCount = Number(match[9] ?? match[10] ?? 0) || null;
    const direction: "andata" | "ritorno" = /RITORNO/i.test(match[7] ?? match[3]) ? "ritorno" : "andata";
    return {
      ...
      direction,
      lineAmount: parseEuroAmount(match[8]),
      pax: passengerCount,
      totalAmount: parseEuroAmount(match[11]),
      service: {
