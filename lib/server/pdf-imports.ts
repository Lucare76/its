...
export function buildPdfImportDetail(row: InboundRow, linkedService: ServiceRow | null, statusEvents: StatusEventRow[]): PdfImportDetail {
  ...
  return {
    ...
    status,
    agency: clean(String(normalized.agency_name ?? "")),
    customer: clean(String(normalized.customer_full_name ?? "")),
    arrival_date: clean(String(normalized.arrival_date ?? "")),
    hotel_or_destination: clean(String(normalized.hotel_or_destination ?? "")),
