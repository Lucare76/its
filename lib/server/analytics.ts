...
export async function resolveContext(request: NextRequest): Promise<AnalyticsContext | NextResponse> {
  ...
  const membership = resolvePreferredMembership(membershipRows, tenantContext.preferredTenantId) as { tenant_id: string; role: Role } | null;
  if (!membership) {
    return NextResponse.json({ error: "Membership non trovata." }, { status: 403 });
  }
...
export async function computeAnalytics(
  ...
  const { data: servicesData, error: servicesError } = await admin
    .from("services")
    .select("id, date, time, vessel, hotel_id, customer_name, status, direction, pax, final_price_cents, margin_cents, agency_id")
  ...
  const agencyMap = new Map<string, { name: string; services: number; pax: number; revenue_cents: number }>();

  for (const service of services) {
    const rev = service.final_price_cents ?? 0;
    const margin = service.margin_cents ?? 0;
    const pax = service.pax ?? 0;
    totalRevenueCents += rev;
    totalMarginCents += margin;
    totalPax += pax;
    if (service.status === "cancelled") cancelled++;
    if (service.status === "problema") problemi++;

    // trend giornaliero
    const day = dailyMap.get(service.date) ?? { date: service.date, services: 0, pax: 0, revenue_cents: 0 };
    day.services++;
    day.pax += pax;
    day.revenue_cents += rev;
    dailyMap.set(service.date, day);

    // top agenzie
    if (service.agency_id) {
      const ag = agencyMap.get(service.agency_id) ?? { name: service.agency_id, services: 0, pax: 0, revenue_cents: 0 };
      ag.services++;
      ag.pax += pax;
      ag.revenue_cents += rev;
      agencyMap.set(service.agency_id, ag);
    }
  }

  const cancellationRate = services.length > 0 ? Math.round((cancelled / services.length) * 1000) / 10 : 0;
  const problemaRate = services.length > 0 ? Math.round((problemi / services.length) * 1000) / 10 : 0;
  const avgRevenueCents = services.length > 0 ? Math.round(totalRevenueCents / services.length) : 0;
  const dailyTrend = Array.from(dailyMap.values()).sort((a, b) => a.date.localeCompare(b.date));

  // Risolvi nomi agenzie
  const agencyIds = Array.from(agencyMap.keys());
  const { data: agenciesData } = agencyIds.length > 0
    ? await admin.from("agencies").select("id, name").eq("tenant_id", tenantId).in("id", agencyIds)
    : { data: [] };
  for (const ag of (agenciesData ?? []) as Array<{ id: string; name: string }>) {
    const entry = agencyMap.get(ag.id);
    if (entry) entry.name = ag.name;
  }
  const agencyRank = Array.from(agencyMap.entries())
    .map(([agency_id, v]) => ({ agency_id, agency_name: v.name, services: v.services, pax: v.pax, revenue_cents: v.revenue_cents }))
