export interface Database {
  public: {
    Tables: {
      ...
      services: { Row: { id: string; tenant_id: string; inbound_email_id: string | null; is_draft: boolean; date: string; time: string; service_type: "transfer" | "bus_tour"; direction: "arrival" | "departure"; vessel: string; pax: number; hotel_id: string; customer_name: string; created_by_user_id: string | null; phone: string; phone_e164: string | null; notes: string; tour_name: string | null; capacity: number | null; meeting_point: string | null; stops: string[] | null; bus_plate: string | null; share_token: string | null; share_expires_at: string | null; reminder_status: "pending" | "sent" | "delivered" | "read" | "failed" | null; message_id: string | null; sent_at: string | null; status: "needs_review" | "new" | "assigned" | "partito" | "arrivato" | "completato" | "problema" | "cancelled" } };
