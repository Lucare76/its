...
export const roleSchema = z.enum(["admin", "operator", "driver", "agency", "supervisor", "assistenza", "autista"]);
...
export const agencyBookingCreateSchema = z
  .object({
    ...
    medmar_infant_count: z.number().int().min(0).max(16).default(0),
    medmar_child_count: z.number().int().min(0).max(16).default(0),
    medmar_adult_count: z.number().int().min(0).max(16).default(0),
  ...
  .superRefine((value, ctx) => {
    ...
    if (value.booking_service_kind !== "formula_snav") {
      if (!value.customer_first_name || value.customer_first_name.trim().length < 2) {
        ctx.addIssue({
          code: z.ZodIssueCode.custom,
          message: "Nome cliente obbligatorio (min 2 caratteri).",
