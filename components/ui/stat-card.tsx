...
export function StatCard({ label, value, hint, loading = false }: StatCardProps) {
  ...
  const valueClassName = isNumeric
    ? "mt-2 text-[2rem] font-bold leading-none tracking-[-0.02em] text-text sm:text-[2.2rem] lg:text-[2.4rem]"
    : "mt-2 text-[1.5rem] font-bold leading-tight tracking-[-0.01em] text-text sm:text-[1.7rem]";

  return (
    <article className="card kpi-card h-full min-h-[196px] p-3 sm:min-h-[208px] sm:p-4">
      <div className="kpi-top-line" />
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          <p className="line-clamp-2 text-[11px] font-medium uppercase leading-4 tracking-[0.03em] text-muted [hyphens:none] sm:text-xs">
            {label}
          </p>
          {loading ? (
            <div className="mt-2 h-10 w-24 animate-pulse rounded-lg bg-slate-200/70" />
          ) : (
            <p className={`break-words ${valueClassName}`}>
              {displayValue}
            </p>
          )}
        </div>
      ...
      {loading ? <div className="mt-3 h-4 w-3/4 animate-pulse rounded bg-slate-200/70" /> : <p className="mt-2 line-clamp-2 text-xs leading-4 text-slate-500 sm:text-[13px]">{hint}</p>}
      <div className="mt-3 h-6 overflow-hidden rounded-lg border border-border bg-surface-2/70 px-2 py-1.5">
